/* A Doom-like slideshow: frames rendered one pixel per token inside a tick loop that turns
   the view by the tick's input (Stage D + E1 shape). The outer loop is the game loop (the tick
   kernel: heading += turn), the inner loop the frame (the pixel kernel). Originally (Stage E1 shape, 16-bit): the token packs
...
static u16 t, col, row, heading, ang, d, h, top, bot, a, b, c, i, f, turn;
