...
def test_dark_request_rail_replays_the_next_word_and_actd_relights_it():
    ...
    relay = roles.index("m.r2.relight.m.r1.edge")
    ...
    relight = (topo.src == relay) & (topo.dst == u)  # the veto-gated re-light relay's ignition
            ...
            for k in range(before, len(self._spk_step)):
                ...
                for node, neuron in zip(self._spk_node[k].tolist(), self._spk_neuron[k].tolist()):
                    ...
                    if neuron == cell.start and not self.injected[node]:
                        ...
                        # at +68.9 ms, 5.3 ms before the 14-hop repair tap. (A 3 x 1.5 kill
                        # train, tried and reverted on 2026-09-20, left the rail ~3 ms slower
                        # to re-ignite and needed +56 ms here.)
