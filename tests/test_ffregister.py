...
rail pulse through a per-rail set chain and cleared by the reset train aimed at their u
members. The default build is untouched (tests/test_handshake.py etc. run on it).

Fast: 200 random transfers with zero decoded errors; the CLEAR at every phase; a stray
pulse that puts a parked rail into lockstep is healed by the next reset; a 20-transfer
mix-B smoke of the campaign loop below. Slow (cluster only): 10,000 transfers at mix B.

    python tests/test_ffregister.py     # re-measures docs/contracts/ffregister.yaml
"""
...
from drosophilos.protocol.handshake import add_register, build_channel
...
def test_flipflop_register_200_random_transfers():
    ...
    for k in range(4):
        ...
        # every set chain fires exactly once per active rail, and no consumer rail's u
...
def test_register_clears_at_every_phase_and_holds_two_seconds():
    ...
    assert len(R.all_flipflops()) == 8 and len(flipflop_taps(net)) == 8
    word = 0b1010
    ...
    train = _spikes(sim, R.rails[1][1].u, 6000, 26000)
...
def test_mix_b_10000_transfers():
    ...
    arrival jitter <= 100 steps), 200 nodes x 20 transfers per chunk. Cluster only."""
...
def measure_ffregister_contract() -> dict:
    ...
    c = measure_contract("ffregister_4bit", ch, PARAMS, words, runner=runner,
                         timing_assumptions=[
                             ...
                             "power-on: one loop-strength inhibitory pulse into every flip-flop's u with the image (Channel.power_on_events); without it a pair fires in lockstep for ever",
                             "loads (upstream DATA) arrive only after READY",
                         ],
                         notes=["4-bit transport channel, consumer rails on flip-flops (storage='flipflop'), flags (valid, tree, completion, fault) on latches by measurement, watchdog 55 hops",
                                ...
                                "status: measured 2026-09-16, not frozen; mix-B campaign pending (cluster)"])
    ...
    # reset latency: Q reset trigger -> last spike of any rail u (CLEAR done) and -> READY
    us = [ff.u for pair in Q.rails for ff in pair]
    clear = [((ev["step"][np.isin(ev["neuron"], us) & (ev["step"] >= r.reset_step) & (ev["step"] < r.ready_step)].max()) - r.reset_step) * PARAMS.dt for r in recs]
    c["reset_latency"] = {"clear_ms_mean": round(float(np.mean(clear)), 1), "clear_ms_max": round(float(np.max(clear)), 1),
                          ...
                          "set_train_ms": "u's first spike 6.0-6.5 ms after the set trigger's pulse (docs/a1_flipflop.md)"}
    ...
    c["error_rate"] = {"fast": {"transfers": 200, "decoded_errors": 0, "upper_95": clopper_pearson_upper(0, 200),
                       ...
                       "mix_B_10000": "pending: tests/test_ffregister.py::test_mix_b_10000_transfers (slow, cluster)"}
    c["campaign"] = None
    return c
...
if __name__ == "__main__":
    ...
    c = measure_ffregister_contract()
    write_contract(c, Path("docs/contracts/ffregister.yaml"))
    print("wrote docs/contracts/ffregister.yaml:", c["resources"]["neurons"], "neurons; accept", c["latency"], "cycle", c["initiation_interval"])
