...
def test_true_guards_reject_a_dark_commit_pair_when_both_ignitions_are_lost():
    """The default build keeps the stage correct without the legacy re-ignition path."""
    pl = _kernel(true_guards=True)
    a = pl.cells[0]
    topo = pl.net.topology()
    ...
    sim = TorchSim(topo, P, n_nodes=1, device="cpu", quanta=quanta)
    outs, _, st = run_pipeline_batched(pl, P, [[1]], max_ms=30000, device="cpu", progress=0,
                                       sim=sim, capture_spikes=(0, [pulse]))
    steps, neurons = (np.asarray(x) for x in st["captured_spikes"])
    commits = steps[neurons == pulse]
    assert [v for _, v in outs[0]["b"]] == [2]
