...
| §4 Track B (specialized cells) | **B1+B2 merged 2026-09-19** (`f1806a0`, openai-sol): `build_pipeline(datapath="specialized")` for AND/OR/XOR/MOV and LOAD; per 8-bit cell 3,172 → 2,070–2,198 neurons, ~35 % fewer spikes, latency −8 ms of ~1,150; flags per transaction; STORE stays generic (its RAM-write margin is the ALU path); cluster matched comparison (`--variant-datapath specialized`) and the 100-copy noisy campaign not yet run | `docs/track_b.md`, `tests/test_kernel_specialized.py` |
| §5 multiplier diagnostic | tool built 2026-09-19 (`bench/mul_diag.py`); perspective/pipelined standalone captured (Juno 413027): each row cycles in ~1.5 s — 590 ms compute, 530 ms commit+reset, 80 ms consumer wait — and 15 of 16 tokens overlap; doom4 8×5×1 array vs pipelined captures running (413025, 413026) | `docs/perf/muldiag_persp_pipelined.json`, `tests/test_mul_diag.py` |
