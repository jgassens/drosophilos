...
| §4 Track A (simulator) | profiled 2026-09-19 (Juno 412204: step cost flat 0.36–0.46 ms from 3k to 880k neurons; launches and two host syncs, not data); FastSim + Observer + K-step blocks merged 2026-09-19 (`70f7267`, claude-opus after openai-sol stalled), bit-identical on CPU/MPS; H200 A/B, profiled and eager/scatter twins queued (Juno 413020–413022); Kimi review running | `docs/perf/juno-h200-profile.md`, `docs/track_a.md`, `sim/lif_fast.py`, `sim/observe.py`, `tests/test_sim_fast.py` |
| §4 Track B (specialized cells) | dispatched 2026-09-19 (openai-sol): `build_pipeline(datapath=)`, B1 AND/OR/XOR/MOV, B2 LOAD/STORE with the STORE completion margin kept | — |
| §5 multiplier diagnostic | tool built 2026-09-19 (`bench/mul_diag.py`: per-token wait_input / go / compute / commit_wait / drain from captured handshake events, row overlap for MULP); doom4 8×5×1 array vs pipelined and the perspective kernel queued on Juno (413025–413027) | `tests/test_mul_diag.py` |
| §6 tick-failure diagnosis, 1,000-tick Stage D exit | not started (in parallel; not part of the speed work) | — |
